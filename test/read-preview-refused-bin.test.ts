 ...
 * dispatcher refuses it before any handler runs: exit 2, saying why. Replaces
...
describe("--preview on a command that does not declare it", () => {
  it("is refused with exit 2 and the write-only message, on every such command", async () => {
    ...
    for (const c of without) {
      ...
      if (r.code !== 2 || !r.stderr.includes("--preview is only valid on write commands")) {
